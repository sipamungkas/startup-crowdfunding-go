...
type service struct {
	transactionRepositry transaction.Repository
...
func (s *service) ProcessPayment(input transaction.TransactionNotificationInput) error {
	...
	transaction, err := s.transactionRepositry.GetByID(transactionID)
	if err != nil {
		return err
	}
	...
	updatedTransaction, err := s.transactionRepositry.Update(transaction)
	if err != nil {
		return err
	}
